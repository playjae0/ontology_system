# -*- coding: utf-8 -*-
"""칸 0.4 — 게이트웨이 연결 확인 — **붙었는가를 한 줄로 묻는 명령** (문서 7 §7.6-B).
...
만들지 않는다(`core/llm/gateway.py::key_state`). 화면 캡처가 밖으로 나갈 수 있다.

**환경변수 이름조차 이 파일에 적지 않는다** — 설정 접근의 수렴(§7.6-B-1)은
읽는 코드만이 아니라 **아는 코드**를 세는 규율이고, 회귀가 이름으로 센다.

**판정 자체는 `core/llm/gateway.py::probe()`가 한다** — 게이트웨이 주소·인증·경로를 아는
...
def main(argv):
    ...
    print(f"  {check.mock_state()} (이 명령은 값과 무관하게 실호출을 시도한다)")
    print(f"  API 키 {check.key_state()}\n")

    points = list(gateway.POINTS) if every else []
    if every:
        print(f"  --all: 지점 {len(points)}종을 각 1회씩 더 부른다 (비용)\n")

    stages = check.probe(points)
    fatal = None
    for s in stages:
