...
# — 규약 10(공용 코어 호출) 전환 완료. 148→109줄·산출 바이트 동일 실측.
# 여기는 **모범 전시장**이다 — 생성 세션이 few-shot으로 읽는다.
# — 전시 목적으로 원본에 **열 하나(K 개정일 · role: meta)를 더했다**(B72 ①):
#   「meta도 role이고, 여러 meta 열을 `meta` 딕셔너리로 묶지 않는다」의 실례가
#   가장 단순한 table 예시에 없었다. 원본(실행 자산)은 손대지 않았다 — 표본
#   CP01.xlsx에 없는 열이라, 원본에 더하면 역산 정합·CSV 등가까지 흔든다.
"""칸 1.4 — 참조 cp 어댑터 — S11(지문 스캔)의 재료.
...
ADAPTER = {
    ...
    "expects": {
        ...
        "columns": {
            ...
            # **meta도 role이다** — 열마다 fields에 선언한다. 여러 meta 열이 있어도
            # `meta` 딕셔너리 하나로 묶지 않는다(table 레코드에 그 고정 키는 없다).
            "개정일": "K",
        },
