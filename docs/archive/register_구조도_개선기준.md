...
> **지위**: 이것이 **되어야 하는 모습**이다(to-be). 현재 실물은 `docs/구조도/01_등록_흐름.md`(B51에서 `register_구조도.md`를 대체 — as-is — 끊긴 자리 3곳 표시)가 정확하고, 그 판은 **B29 완료의 대조 기준으로 보존**한다. 구현 완료 보고가 오면 이 문서가 현행 구조도가 되고, as-is 판은 머리에 「역사 — B29 이전」을 단다.
