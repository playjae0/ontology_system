...
| # | 파일 | 왜 |
|---|---|---|
| 1 | `layers/process/config.json` (2.7KB) | **양식 그 자체** — 19키의 실제 형태 |
| 2 | `layers/quality/config.json` (2.7KB) | **두 번째 예** — 무엇이 층마다 다르고 무엇이 같은지 보인다. 그리고 **비워도 되는 키**(canonical_scope·polarity·skeleton_version 없음)를 실물로 보여준다 |
