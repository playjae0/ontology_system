...
> **결론 먼저**: config 키는 19종이지만 **사람이 실제로 쓰는 것은 11종**이고, 그중 **절반은 비워도 돈다**(품질층 실물이 증거 — `canonical_scope`·`polarity`·`skeleton_version` 없이 돈다). 나머지 8종은 베끼거나 자동으로 따라온다.
...
**`categories`** — 닫힌 목록 + 정의문. **정의문 규격: "무엇인가"만이 아니라 "무엇과 헷갈리는가"를 담는다.**
...
### 초안 프롬프트 (1단계용 — 복사해서 쓴다)

```
당신은 지식 그래프 층의 카테고리를 설계한다. 출력은 JSON 하나뿐이다.

[입력] 층 설명 1문단 + 이 층의 대표 문서 (아래 붙임)
[출력] {"categories": {"이름": "정의문", …}}  — 2~5종이 정상이다. 많으면 잘못 갈랐다.

[규율]
1. 정의문은 "무엇인가" + "무엇과 헷갈리는가(제외 서술)"를 반드시 담는다.
2. 후보마다 자문한다: ①이름으로 지칭 가능한가 ②다른 정보의 경유지인가
   ③개수가 문서 비례인가 — 시간 비례(측정값·로그)면 카테고리가 아니다.
3. 값·수치·판정결과는 카테고리가 아니다 — attribute다.
4. 판단이 갈린 후보는 JSON 뒤 "확인 필요:"에 적는다.
```
