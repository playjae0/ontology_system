...
| 경로 | 만드는 것 | 도구 | 상태 |
|---|---|---|---|
| **① 구축 모드** (doc_type 등록 — 문서 6 §6.5~6.7) | 어댑터 + 매칭 스키마 쌍 | **지시문 `prompts/1.4_generate.md`(v1.5) + `kit/` 5종 — 이미 실물** (스켈레톤 · 실행 하네스 · 렌더러 · 뷰 스키마 · 참조어댑터). 3차 블라인드 통과 | ✅ 새로 만들 것 없음 — 표본 넣고 검수·승인 1회 |
...
**`categories`** — 닫힌 목록 + 정의문. **정의문 규격: "무엇인가"만이 아니라 "무엇과 헷갈리는가"를 담는다.** **정의문은 5요소로 쓴다 — ①핵심 정의 한 문장 ②실물 예시 5~10(골격·문서에 실재하는 이름만) ③헷갈림 규칙 3~5(「X처럼 보이지만 Y다」) ④열 이름 패턴 ⑤이 카테고리가 아닌 것. 정의문이 곧 판정 프롬프트다 — 교과서 문장이면 판정도 교과서 수준이 된다(실측). 초안은 `층정의_프롬프트.md`로 사내 LLM에서 뽑고 사람이 골격 대조 후 확정 — 신규 층·기본층 보강 모두 같은 프롬프트다.**
...
### 1-a. 있는 층의 어휘를 넓힐 때 — 카테고리 추가 (설비 사례 · 2026-09-22)

새 문서 종류(RFQ 등)가 지금 어휘로 안 잡히면 **층을 새로 만들지 말고** 그 층의 config를 넓힌다. 카테고리·관계는 **추가**라 기존 그래프를 깨지 않고, 재인입은 새 문서만이다. 설비 사례에서 손대는 자리:
...
| `config_version` | 올린다 | 층 개정 절차 — 어느 판으로 넣었는지 대장에 남는다 |

적용: config 저장 → `python run.py bootstrap`(fresh 없이 — 골격 불변이면 아무것도 안 바뀐다) → 새 doc_type 등록 → 인입. **골격(skeleton.json)은 손대지 않는다** — 골격 노드가 공정이자 설비다(골격작성_가이드 §3).
