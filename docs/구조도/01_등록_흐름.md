...
> **무엇**: 패키지 `cli/register/`(B78-2 — generate·draft·gate·interview·ledger·view·confirm · 진입점 `python -m cli.register`)의 3단(생성·검수·확정)이 어떤 함수를 거쳐 무엇을 주고받는지. **B48(파서 주입)·B49(전 열 판정)·B50(하네스→생성)·[정정] 40·B55(지시 템플릿·문답 누적)·B58(재등록·관문 범위·고정 prose 어댑터·형태 판정·뷰를 생성이) 반영본**이다.
> **정본**: 문서 6 §6.4~6.7 · 문서 1 M9·C19. 이 문서는 설명서다 — 갈리면 명세가 이긴다.
> **실물**: `cli/register/{generate,draft,gate,interview,ledger,view,confirm}.py` · `cli/interview.py` · `cli/prompt.py` · `parser/form.py` — 원격 `55f9c59`(B58 ⑤) 기준. **행 번호는 적지 않는다** — 밀린다. `grep -n`으로 찾는다.
...
flowchart TD
  ...
  subgraph GEN["① generate — cmd_generate"]
    direction TB
    PKG["input_package.json<br/>human 4 + system 5<br/>hint.interview = <b>batches {samples, at, stale?, decisions[]}</b><br/><b>rounds → interview_log.json (B62 ②)</b> · old package migrates + warns if no decisions"]
  ...
  subgraph MG["machine gate — machine_gate()  (B50)"]
    direction TB
    FJ{"⚙ form judge (C37)<br/>auto? else 👤 [table/prose] (B65 ⑤)<br/>prose → basic adapter · LLM 0"}
    FJ -- "table" --> ST
    FJ -- "prose / --use-basic" --> BASIC
    ST["⚙ stamp_system_fields (B62 · B64)<br/>working copy: header_labels ← sample · adapter_version ← 1.{rev}<br/><b>columns: label / list → letters (B64 ①)</b><br/>header_row_suspect → FAIL"]
    HS["⚙ harness · kit/run_adapter.py<br/>① load (+G1B static vocab — stop here if red, B65 · +G4F shape table, B76) ② preflight ③ extract (+G39 output keys ⊆ schema fields, B72 · +G3A cell-None mutation · G31 says file:line, B76) ④ schema (+G4C/G4D/G4E layer vocab, B65 · +G4G ledger covers all fields, B76)<br/><b>⑤ pipeline.parse — full parser (B58 ②)</b><br/>LLM points injected as no-op"]
    ...
    MORE{"still FAIL after 1?<br/>👤 더 돌릴까 [Y/n] (B62 ④)<br/>same FAIL repeats → stop"}
    ST --> HS --> OR --> CL
  ...
  DR --> ST
  BASIC --> ST
  BASIC -. "--instruct / --revise" .-> NOREGEN["✗ 재생성 대상이 아니다 (B65 ④)<br/>next: basic_*.py 상수 · schemas/<dt>.json · --as --no-basic"]
  ...
  subgraph CONF["③ confirm — cmd_confirm"]
    direction TB
    PR["_promote<br/>registry/review/ → registry/adapters/{doc_type}.py · registry/schemas/{doc_type}.json"]
    RG2["registry.register → <상태>/registry/doc_types.json<br/>approval.json: by · at · instructions"]
    PR --> RG2
  end
  H4 --> PR
  RG2 --> NEXT["next: ingest-file --doc-type dt --dry-run → ingest-file (B66 ③)"]

  subgraph KIT["📄 kit/ — assets that shape generation"]
    direction TB
    K1["prompts/1.4_generate.md (B63 — 판 하나)<br/>rules · role 6 · unmappable · output spec"]
...
| 관문 PASS 뒤 파이썬 예외로 죽었다 | **형(G4F)·대장 커버리지(G4G)·셀 None(G3A)을 관문이 먼저 재고**, 그래도 나는 예외는 `[결함] 파일:줄` 한 줄 + `defects.log` | LLM 산출의 모양이 흔들리는 것은 정상이고 그것을 잠그는 자리는 코드가 아니라 관문이다(B76 · 사내 실측 19·20) |
...
|---|---|---|---|
| 0 | `--resume` (`cmd_generate` resume 갈래) | **이어하기 = 같은 입력 + 열 판정 대장 + 지난 실패**(B67 ①). 초안이 있으면 **관문부터** — FAIL이면 그 문면과 `instructions` 이력을 지시로 실어 revision+1 draft, PASS면 draft 0(통과한 초안을 갈지 않는다). 초안 없으면 초회와 같은 입력. 층·표본 인자는 무시(경고 1줄). 처음부터 = `--resume` 없이 | `review/<dt>/columns.json` |
| 0″ | **`samples.sample_roles`** — 표본의 시트 관문(B86 ⑤) | 입구 가드 **다음, 형태 판정·초안 앞** — 시트 여럿인 산문 엑셀 표본마다 `cli/sheet_gate.gate()`(인입과 같은 함수·표·질문). 답은 `registry/sheet_roles/<doc_id>.json`(인입과 같은 기록 — 나중 인입은 「기록대로」). 비대화형은 상태 거부 + `generate … --sheets "…"` 줄 · `--sheets`는 표본 하나일 때만. 표로 판정된 표본은 건너뛴다 | `registry/sheet_roles/<doc_id>.json` |
...
| Step | Function | Does |
|---|---|---|
| a | `harness()` → `kit/run_adapter.py` | ①로드(문법·순수성·규약 10) ②preflight(양식 지문) ③extract 실행 ④스키마 정합 **⑤`pipeline.parse` 전 구간**(tagger·validator — LLM 지점은 무동작 주입 · B58 ②). **킷은 `core`를 모른다 — 자리는 플래그로 받는다**(B86): `--closed-list <상태>/data/skeleton_closed_list.json`(골격 목록 · 없으면 좌표 대조 생략 한 줄) · `--sheet-roles <임시 표>`(표본 절대 경로 → 역할 · `skip` 시트는 킷도 안 본다) · 구조 지도 보존 자리는 킷 안의 임시 폴더(코드 폴더에 남는 파일 0). **통과하면 검수에서 기계 오류가 날 자리가 없어야 한다 — 남으면 결함** |
...
| g′ | **`status`·`confirm`은 `regate()` → `machine_gate(fix=False)`**(B60 ①) | 저장값을 안 믿고 그 자리에서 다시 판정. LLM 0. 저장값 PASS라도 어댑터가 바뀌었으면 FAIL로 막는다 |
...
조건 셋 — `state.json` 존재 · `machine_gate == PASS` · `--by`. `_promote`가 `review/` → `adapters/{dt}.py`·`schemas/{dt}.json`으로 **복사**하고, `registry.register`가 상태 루트 `registry/doc_types.json`에 등재(경로 값은 `registry/` 기준 상대 — B78), `approval.json`에 승인자·시각·지시 이력(사람·자동 구분). **원본은 안 지운다.**
...
# 다시 만들 때
python -m cli.register generate <doc_type> --resume            # 이어하기 = 같은 입력 + 대장 + 지난 실패 (관문부터)
...
| `<상태>/registry/doc_types.json` | 등록부 — `revision` 포함 · ②등록 단 | `register list` |
