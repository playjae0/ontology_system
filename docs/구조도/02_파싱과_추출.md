...
`ingest-file <문서>` / `ingest-dir <경로>`가 위 셋을 순서대로 부른다 — 같은 함수, 같은 산출물. `--doc-type` 없으면 `cli/scan.py`가 헤더 지문으로 후보를 내고 **유일 일치면 자동**(근거를 `doc_registry.routing`에 기록), 둘 이상·0건이면 사람. **doc_id = 파일명 stem(공백→`_`), 경로 무관**(D-110) — 같은 파일명은 같은 문서 = 개정판 재인입. `--dry-run`으로 선택만 미리 본다. **형태 판정(⓪)이 그 앞에 돈다** — 화면에 「형태 판정 + 신호값」 두 줄이 먼저 뜨고, 그 결과가 `routing.form`에 남는다.
