...
flowchart TD
  subgraph CLI["operator commands"]
    ...
    PR["run.py parse run adapter doc_id doc"]
...
|---|---|---|---|
| ① | `reader` | 포맷별 원시 추출. **빈 셀은 읽지 않는다**(`reader.py:57`) | ImportError는 지연 import로 격리 |
...
| ④′ | `tagger.complete_images` | 이미지 요약 — 주입된 `summarize` 또는 고정 문자열 | |
