...
**시트 역할 관문**(B83): 산문 엑셀이고 시트가 둘 이상이면 파싱 **앞**에서 `registry/sheet_roles/<doc_id>.json`을 본다 — 없으면 시트 전부의 표(행·열·글자 비율·앞부분·규칙 제안)를 찍고 한 줄을 묻는다(tty) · 비대화형은 시트 이름 전부를 문면에 싣고 거부. 답 `{시트: prose|ref|skip}`은 파서 진입점 인자 `sheet_roles`로 들어가(문서 6 §6.4) `skip`은 안 읽고 `ref`는 청크 `meta.sheet_role="ref"`로 저장만(추출 0 · `linked=false` · `answer()` 근거 0). 파서는 기록을 읽지 않는다 — CLI가 읽어 넘긴다.
