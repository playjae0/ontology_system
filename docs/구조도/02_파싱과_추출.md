...
| ⑦ | `struct_map` | **레벨은 인입마다 결정적 규칙이 고른다**([정정] 46 — 구판은 등록 때 상수로 박았다): `level_stats`의 레벨별 avg 행수에서 **목표 구간 [5,40] 안에 들고 구간 중앙에 가장 가까운 레벨.** 규칙이 못 정하는 계열만 **LLM이 제목 행·급을 판정**(`map_structure`)하고 **분할은 언제나 코드**다. `source: live/heuristic/kept` · `extract/struct_maps/`에 보존. **무엇을 보고 골랐나가 기록된다**(B68): 프레임마다 `분할_기준`·`지도_출처` — 고정 산문 xlsx 어댑터는 `adapter:basic_prose_xlsx` · 신호 넷(번호 → 가로병합 → 굵게+들여쓰기 → 없음)의 헤딩 수, 지도 경로는 `구조 지도(LLM · 판본 n)`/`heuristic`. 두 경로 한 이름 — generate/status가 프레임당 한 줄, 뷰 레벨 선택 표에 「분할 기준」 열 | 예산 초과·판정 불가 → 평면 폴백 + `hierarchy_unresolved`(`case: flat_fallback`) · **구간 밖이면 최근접 레벨 + `hierarchy_unresolved`(`case: size_out_of_band`)** — 자동으로 지도 패스(LLM)로 넘기지 않는다([정정] 48) |
