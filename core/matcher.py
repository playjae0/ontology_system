...
def resolve(surface, category, layer, graph, dictionary, *, scoped=True,
    ...
    `polarity`는 극성 결합된 표면형의 극성값이다(무결합이면 None). **극성이 다르면
    후보에서 제외한다** — "cathode 노칭 프레스"와 "anode 노칭 프레스"는 표기 차이가
    아니라 다른 실물이고 다른 규격을 갖는다(CH3A 3.3 경계). 문자열 포함 규칙에
    맡기면 극성 노드가 무극성 노드에 흡수되므로, 이것은 휴리스틱이 아니라
    정체성 규칙으로 건다.
    """
    ...
    for nid in dictionary.get(norm(surface), []):
        ...
        if n and n["category"] == category and n["layer"] == layer \
                and n.get("electrode_type") == polarity:
    ...
    for nid, n in graph.nodes.items():
        ...
        if n.get("electrode_type") != polarity:
