...
# ---------------------------------------------------------------- ⓪원본
def raw(*parts):
    """원본 문서 — `$ONTO_HOME/raw/` (B79 ② · 이름은 B80 ②). **사람이 넣고 시스템은 읽기만.**
    ...
    **이름이 `docs/`였다가 `raw/`가 됐다**(사용자 지적 2026-09-21): 레포의 `docs/`는
    명세 폴더라 사람이 헷갈렸고, 이관이 그 둘을 가르려고 확장자 예외까지 두게 됐다
    (D-160 ④) — 이름이 나쁘다는 증거다. 이름이 갈리자 그 예외가 사라졌다.

    `ingest-dir`가 인자 없이 돌면 이 폴더 전체를 **재귀로** 돈다.
    """
    return _under("raw", *parts)


def legacy_raw():
    """원본 자리의 **옛 이름** — `$ONTO_HOME/docs/` (B79 ② → B80 ②).

    이름이 바뀐 자리를 아는 것도 자리 소유자의 일이다: 옛 이름에 문서를 넣어 둔
    사람에게 「0건」이 아니라 **옮기라는 문면**을 주려면 그 이름이 어딘가에 있어야
    하고, 그 어딘가가 여기여야 이름이 또 바뀔 때 한 곳만 고친다.
    """
    return _under("docs")
...
def rel_to_home(p):
    ...
    """
    q = Path(p)
    # **경로가 아닌 표기는 그대로 둔다** — mock 픽스처의 「(mock)」처럼 자리를
    # 가리키지 않는 값을 절대화하면 화면이 없는 파일을 가리킨다(B80 ②).
    if not q.is_absolute() and "/" not in str(p) and not q.exists():
        return str(p)
    try:
